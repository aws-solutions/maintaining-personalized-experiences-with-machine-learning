...
from aws_solutions.scheduler.common import TASK_PK
...
class Scheduler:
    ...
    def __init__(self, table_name=None, stepfunction=None):
        self.ddb = get_service_resource("dynamodb")
        self.ddb_cli = self.ddb.meta.client
        self.table_name = os.environ.get("DDB_SCHEDULES_TABLE", table_name)
    ...
    def is_enabled(self, task: Task) -> bool:
        arn = self._get_running_execution_arn(task)
        return True if arn else False
    ...
    def deactivate(self, task: Task) -> None:
        self._disable_schedule(task)
