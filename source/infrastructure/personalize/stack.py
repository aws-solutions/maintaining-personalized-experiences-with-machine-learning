...
class PersonalizeStack(SolutionStack):
    def __init__(self, scope: Construct, construct_id: str, *args, **kwargs) -> None:
        ...
        # Manually add a dependency between the data bucket's notification and policy settings
        # to prevent a race condition between the settings during resource tear down.
        data_bucket.node.find_child("Notifications").node.add_dependency(data_bucket.node.find_child("Policy"))
